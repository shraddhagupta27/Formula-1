...
race_frequency = circuit_races.groupby(["country", "year"])["raceId"].count().reset_index()
...
top_countries['Cumulative_Race_Count'] = top_countries.groupby('country')['race_count'].cumsum()
...
for year in year_ranges:
    ...
    country_race_count = filtered_data.groupby("country").size().reset_index(name="race_count")
...
choropleth_fig.update_layout(
    ...
    coloraxis_colorbar=dict(
        ...
        ticks="outside",
        tickvals=[0, 20, 40, 60, 80, 100, 120],  # Customize tick values (adjust as needed)
    ),
)
...
top_10_circuits["normalized_color"] = (top_10_circuits["race_count"] - min_count) / (max_count - min_count)
...
race_winners = pd.merge(race_winners, drivers[["driverId", "forename", "surname"]], on="driverId")
race_winners = pd.merge(race_winners, constructors[["constructorId", "name"]], on="constructorId")
...
top_10_constructors = constructor_wins.sort_values(by="wins", ascending=False).head(10)["name"].tolist()
...
treemap_data = (
    ...
    .count()
    .reset_index()
    .rename(columns={"name": "Constructor", "driverName": "Driver", "positionOrder": "Wins"})
)

# Add total wins for each constructor (needed for grouping in treemap)
constructor_totals = (
    treemap_data.groupby("Constructor")["Wins"].sum().reset_index().rename(columns={"Wins": "Total Wins"})
)
constructor_totals["Driver"] = constructor_totals["Constructor"]  # Placeholder for grouping
...
constructor_races.rename(columns={'name': 'constructor', 'raceId': 'total_races'}, inplace=True)

# Identify the top 10 constructors by total races
top_10_constructors = constructor_races.sort_values(by='total_races', ascending=False).head(10)['constructor'].tolist()
...
# Aggregate wins for each constructor
constructor_wins = race_winners.groupby('constructorId')['positionOrder'].count().reset_index()
constructor_wins.rename(columns={'positionOrder': 'wins'}, inplace=True)

# Merge with constructors to get names
constructor_wins_with_names = pd.merge(constructor_wins, constructors[['constructorId', 'name']], on='constructorId')

# Sort by wins and take the top 10 constructors
top_10_winners = constructor_wins_with_names.sort_values(by='wins', ascending=False).head(10)
...
dash_app.layout = html.Div([
    ...
    html.Button("Reset to Constructors", id="reset-button", n_clicks=0, style={"margin-top": "10px"}),
...
def update_treemap(click_data, n_clicks):
    ...
    if trigger_id == "treemap" and click_data:
        ...
        driver_data = filtered_data.groupby(['name', 'driverName'])['positionOrder'].count().reset_index()
        driver_data.rename(columns={'name': 'constructor', 'positionOrder': 'wins'}, inplace=True)
        fig2 = px.treemap(
            driver_data,
...
driver_performance["podium"] = driver_performance["positionOrder"].apply(lambda x: 1 if x <= 3 else 0)
driver_performance["win"] = driver_performance["positionOrder"].apply(lambda x: 1 if x == 1 else 0)

# Step 3: Aggregate total wins for drivers from 2015 onward
active_driver_wins = (
    driver_performance[driver_performance["year"] >= 2015]  # Filter data for 2015 onward
...
top_10_driver_stats = (
    driver_performance[driver_performance["driverId"].isin(active_driver_wins["driverId"])]
...
def index():
    ...
    return render_template('index.html', choropleth_html=choropleth_html, bar_html=bar_html, tree_html=tree_html, line_html=line_html, barr_html=barr_html)
